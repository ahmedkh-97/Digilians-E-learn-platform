...
import {submitAttemptOnline,getLeaderboard,fetchAttemptsForExamIds,syncRankingAvatarProfile,fetchRankingProfiles,syncVoucherPrimaryTrack,fetchVoucherPrimaryTracks} from "./online.js?v=0.22.8";
import {buildAggregateLeaderboard} from "./ranking-engine.js";
import {isRankingMode,isVoucherRankingMode,findRankingLevel,buildRankingScope} from "./ranking-scopes.js?v=0.22.8";
...
import {renderTechnicalQuestion,renderTechnicalOption,renderTechnicalRichText,analyzeTechnicalContent,displayTopicForQuestion} from "./technical-content.js?v=0.22.8";
import {recordMistakeOutcome,seedMistake,getMistakes,getMistake,getMistakeSummary,topicWeakness,questionFromMistake,isPracticeableMistakeQuestion,patchMistakeContext,clearMistakesForOwner,removeMistake,shouldRecordMistakeOutcome,isLegacyUnansweredOfficialSeed,MASTERY_STREAK} from "./mistakes.js?v=0.22.8";
import {getAvatarProfile,hasAvatarProfile,renderAvatarInto,openAvatarPicker,avatarMarkup} from "./avatar-profile.js?v=0.22.8";
import {resolveModuleExamId,moduleAssessmentState,shouldSyncAttemptOnline} from "./module-assessment.js?v=0.22.8";
import {createUuid} from "./runtime-compat.js?v=0.22.8";
import {buildExcelTrackResultMetadata} from "./excel-track-results.js?v=0.22.8";
import {resolveLearningFlowExam,buildLearningFlowExamCard,shouldRenderStandaloneTrackExamRow} from "./learning-flow.js?v=0.22.8";
import {
...
} from "./voucher-engine.js?v=0.22.8";
import {
...
function ensurePl300Styles(doc=globalThis.document){
  ...
  link.rel='stylesheet';
  link.href='assets/css/pl300.css?v=0.22.8';
...
async function ensurePl300FullRankedLearning(){
  pl300FullRankedLearning??=await import("./pl300-full-ranked-learning.js?v=0.22.8");
  return pl300FullRankedLearning;
}

async function loadVoucherFullRankedIndex(config=state.voucherExamConfig){await ensurePl300LearningLoop();await ensurePl300LearningController();return pl300LearningController.loadFullRankedIndex(config);}

function voucherFullRankRecord(question){return pl300LearningController?.fullRankRecord(question)||null;}

function voucherFullRankMetrics(){return pl300LearningController?.fullRankMetrics()||null;}

function voucherSourceStartSolveTimer(question,options={}){return pl300LearningController?.startSolveTimer(question,options);}

function voucherSourceConsumeSolveSeconds(question){return pl300LearningController?.consumeSolveSeconds(question)||0;}

function voucherSourceResetSolveTimer(){return pl300LearningController?.resetSolveTimer();}
...
async function syncPl300FullRankSnapshot({force=false}={}){
  ...
  const payload=pl300FullRankedLearning.buildPl300FullRankOnlineAttempt({
    playerId:state.playerId,studentName:state.studentName,examVersion:"0.22.8",metrics,
...
async function openVoucherFullRankedLearning({filter="all",continueIncomplete=false}={}){
  try{
    ...
    voucherSourcePracticeNative??=await import("./voucher-source-practice-native.js?v=0.22.8");
...
function voucherSourceReviewFilteredQuestions(){return pl300LearningController?.filteredQuestions()||[];}

function voucherSourcePracticeRecord(question){return pl300LearningController?.practiceRecord(question)||null;}

function voucherSourcePracticeCorrectIds(question){return pl300LearningController?.correctIds(question)||[];}

function voucherSourcePracticeSelection(question,record=voucherSourcePracticeRecord(question)){return pl300LearningController?.selection(question,record)||[];}

function voucherSourcePracticeSelectionsMatch(question,selected){return Boolean(pl300LearningController?.selectionsMatch(question,selected));}

function voucherSourcePracticeOptionsHtml(question,record){return pl300LearningController?.optionsHtml(question,record)||'';}


function voucherSourcePracticeSummary(){return pl300LearningController.summary();}
...
function voucherActiveSourcePart(){return pl300LearningController?.activePart()||null;}
...
function renderVoucherSourceReview(){
  ...
  const recordStatus=practiceRecord
    ...
    :"NOT STUDIED";
  const revealOpen=!retrying&&!!practiceRecord||state.voucherSourceRevealOpened?.has?.(String(q.id));
  ...
  body.innerHTML=pl300FullRankedLearning.buildPl300FullRankedReviewMarkup({
    ...
    visualHtml,optionsHtml,nativeHtml,revealOpen,answerHtml:voucherSourceReviewAnswerHtml(q,retrying?null:practiceRecord),nextActionLabel:pl300FullRankedLearning.pl300SourceNextActionLabel(practiceRecord)
  });
  ...
  reveal?.addEventListener('toggle',()=>{
    if(!reveal.open||q.reviewMode!=="source-reveal"||(practiceRecord&&!retrying))return;
  ...
  $("sourcePracticeCheckpointBtn")?.addEventListener("click",()=>{
    if(practiceRecord&&!retrying)return;
