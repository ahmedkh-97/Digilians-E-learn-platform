...
replacement=r'''function renderRankedStructuredInputs(q,list,{selected,confirmed,feedbackReady}={}){
  ...
  for(const f of fields){const row=document.createElement('label'),choices=structuredFieldChoices(f),input=choices.length?document.createElement('select'):document.createElement('input');row.className='ranked-structured-field';row.innerHTML=`<span class="ranked-structured-label">${escapeHtml(f.label||f.id)}</span>`;if(choices.length)input.innerHTML='<option value="">Select an answer</option>'+choices.map(c=>`<option value="${escapeHtml(c)}">${escapeHtml(c)}</option>`).join('');else{input.type='text';input.placeholder='Type the source answer'}input.setAttribute('data-ranked-structured-field',f.id);input.value=String(values?.[f.id]??'');input.disabled=locked;if(feedback)input.classList.add(structuredFieldCorrect(f,input.value)?'correct':'wrong');input.addEventListener(choices.length?'change':'input',e=>updateStructuredField(q,f.id,e.target.value));row.append(input);grade(row,f,input.value);list.append(row)}
}
'''
