...
Ordinary localhost testing with `tools/windows/START-LOCAL.bat` does not require Node.js on Windows.

## Fastest way
Double-click `tools/windows/START-LOCAL.bat`.

- `tools/windows/START-LOCAL.bat` does not depend on Node.js, even if Node is installed.
...
Double-click `tools/windows/FULL-QA.bat` to run the complete pre-deploy suite without starting localhost.

## Full acceptance flow
With Node.js 20+ installed, double-click `tools/windows/TEST-LOCAL.bat`. It runs Full QA first; only on PASS does it start localhost and open the browser.

`tools/windows/RUN-PREFLIGHT.bat` is also developer-only and requires Node.js 20+.
